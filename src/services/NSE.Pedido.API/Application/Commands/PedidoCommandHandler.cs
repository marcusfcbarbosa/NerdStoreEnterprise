...
namespace NSE.Pedido.API.Application.Commands
{
        ...
        IRequestHandler<AdicionarPedidoCommand, ValidationResult>
    {
        ...
        public async Task<ValidationResult> Handle(AdicionarPedidoCommand message, CancellationToken cancellationToken)
        {
            ...
            if (!await AplicarVoucher(message, pedido)) return _validationResult;

            // Validar pedido
            if (!ValidarPedido(pedido)) return _validationResult;

            // Processar pagamento
            if (!await ProcessarPagamento(pedido, message)) return _validationResult;
        ...
        private async Task<bool> AplicarVoucher(AdicionarPedidoCommand message, NSE.Pedidos.Domain.Pedidos.Pedido pedido)
        {
            ...
            var voucher = await _voucherRepository.ObterVoucherPeloCodigo(message.VoucherCodigo);
            if (voucher == null)
            {
