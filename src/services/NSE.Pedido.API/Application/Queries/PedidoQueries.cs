    ...
    public class PedidoQueries : IPedidoQueries
    {
        ...
        public async Task<PedidoDTO> ObterUltimoPedido(Guid clienteId)
        {
            ...
            var pedido = await _pedidoRepository.ObterConexao()
                .QueryAsync<dynamic>(sql, new { clienteId });//dynamic como o PedidoDTO nao consegue pegar tudo que a query devolve usar o dynamic
                                                             //pois traz PedidoDTO, PedidoItemDTO,EnderecoDTO

            //outra forma de retornar um objeto complexo pelo Dapper
            //Bem legal
            //var pedido = await _pedidoRepository.ObterConexao()
            // .QueryAsync<PedidoDTO,PedidoItemDTO,EnderecoDTO,PedidoDTO>(sql,map:
            // (p, pi, e) => {
            //     p.PedidoItems.Add(pi);
            //     p.Endereco = e;
            //     return p;
            // },param:new { clienteId },splitOn: "ProdutoId");
